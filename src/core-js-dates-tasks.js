...
function getNextFriday(date) {
  ...
  const daysUntilFriday = 5 - dayNumber + (dayNumber >= 5 ? 7 : 0);
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const differenceInDays =
    Math.round((new Date(dateEnd) - new Date(dateStart)) / millisecondsPerDay) +
    1;
...
function getNextFridayThe13th(date) {
  ...
  for (let month = currentMonth; month <= 11; month += 1) {
    const nextDate = new Date(currentYear, month, 13);
